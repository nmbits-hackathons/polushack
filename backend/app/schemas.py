...
class TechnicsCharacteristic(BaseModel):  # характеристики траспорта
    ...
    operating_weight: int = 20  # максимальная эскплатауционная масса
...
class TechnicsModel(TechnicsCharacteristic):  # характеристики траспорта хранящегося в бд и используемого для выгрузки
    vin: str
    current_place: Optional[str] = "none"  # необходимое месторасположение траспорта
...
class ResponseTechnicsModel(TechnicsModel):  # сущность траспорта в ответе сервера
    id: str
...
class TechnicsSeries(BaseModel):  # серии траспорта
