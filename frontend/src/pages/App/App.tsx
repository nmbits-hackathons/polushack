...
const { Footer: LayoutFooter, Sider: LayoutSider, Content: LayoutContent } = Layout;
...
const App = () => {
  ...
  return <Spin tip="Loading..." size="large" delay={52} style={{ maxHeight: "100vh" }} spinning={isLoading}>
    <Layout style={{ height: "100vh" }}>
      <Layout>
        {isAuth && <LayoutSider theme="light">
          <SideBar />
        </LayoutSider>}
        <Layout>
          <LayoutContent style={{ display: "flex", flexFlow: "column", alignContent: "center", justifyContent: "center", background: "#E8EEF6" }}>
            <Routes>
              {routs.map(({ path, Element, isAuthValue }) => isAuth === isAuthValue && <Route key={path} element={<Element />} {...{ path }}/>)}
            </Routes>
          </LayoutContent>
          <LayoutFooter style={{ boxShadow: "0px 0px 8px rgba(0, 16, 38, 0.08)", display: "flex", justifyContent: "center", alignContent: "center", padding: "12px 12px" }}>
            <Typography.Text>Copyright © 2022   All rights reserved</Typography.Text>
          </LayoutFooter>
        </Layout>
      </Layout>
